#!/usr/bin/env python3
"""Rewrite reviews with proper detailed content."""
import requests, time, json

url = 'https://dancjwbxcycncrkdiyug.supabase.co/rest/v1/reviews'
...
The Lodge 12-inch Cast Iron Skillet remains our top recommendation for most home cooks. Here's why:
...
""",
    ...
    {
        "slug": "best-kitchen-knife-set",
        "title": "Best Kitchen Knife Sets for Home Cooks",
        "excerpt": "A great knife set makes every cooking task easier. We tested the top brands to find the best options for every budget.",
        "content": """## The Best Kitchen Knife Sets

A quality knife set transforms your kitchen. Here are our top picks after testing the best brands.

## Best Overall: Wüsthof Classic 7-Piece Knife Block Set

The gold standard in knife sets.

**Price Range:** $250-300 on Amazon
...
Wüsthof is the gold standard. Every blade is forged from a single piece of high-carbon stainless steel, laser-cut for perfect edges, and beautifully balanced.

[Check Price on Amazon →](https://www.amazon.com/Wusthof-Classic-7-Piece-Knife/dp/B000S6OZOK?tag=provenpantry3-20)
...
## Best Value: Cuisinart 15-Piece Triple Rivet Set

Outstanding value with everything you need.

**Price Range:** $60-80 on Amazon

Cuisinart delivers impressive quality at this price point. The blades are stainless steel (not high-carbon), so they won't hold an edge as long—but they're dishwasher safe and replaceable.

[Check Price on Amazon →](https://www.amazon.com/Cuisinart-15-Piece-Stainless-Steel-Knife/dp/B007VDL8G4?tag=provenpantry3-20)

**Pros:**
- Complete set including steak knives
- Dishwasher safe
- Great value
...
**Essential Pieces:**
- 8-inch chef's knife (your main workhorse)
- 8-inch bread knife (slices crusty bread without crushing)
- 3-4 inch paring knife (detailed work)
- Kitchen shears (herbs, packaging)
...
**Best Overall:** Wüsthof Classic — Lifetime quality.
**Best Value:** Cuisinart — Complete set at great price.
""",
        "published": True,
        "highlights": "7-piece set, Wusthof Classic, Lifetime warranty, Includes block",
        "affiliate_products": "Wusthof Knife Set, Cuisinart Knife Set, Knife Block"
    },
    {
...
# Get existing review IDs and update them
for r in reviews:
    # First, get the id
    get_resp = requests.get(f"{url}?slug=eq.{r['slug']}&select=id", headers=h)
    if get_resp.status_code == 200 and get_resp.json():
        review_id = get_resp.json()[0]['id']
        # Update
        update_resp = requests.patch(f"{url}?slug=eq.{r['slug']}", headers=h, json=r)
        print(f"{update_resp.status_code}: {r['slug']}")
    else:
        print(f"Not found: {r['slug']}")
    time.sleep(0.3)
