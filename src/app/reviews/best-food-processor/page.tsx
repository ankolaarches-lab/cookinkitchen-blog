import Link from "next/link";
import { Metadata } from "next";
import Breadcrumbs from "@/components/Breadcrumbs";

export const metadata: Metadata = {
  title: "Best Food Processors - 2026 Buying Guide",
  description: "Best Food Processor - Complete guide",
  openGraph: {
    title: "Best Food Processor",
    description: "Best Food Processor - Complete guide for home cooks",
    type: "article",
    images: ["/images/reviews/food-processor.jpg"],
  },
};

const products = [
  {
    name: "Cuisinart 14-Cup Food Processor",
    price: "$199.99",
    rating: 4.7,
    link: `https://www.amazon.com/s?k=Cuisinart+DFP+14BCNY+14+Cup+Food+Processor&tag=${process.env.NEXT_PUBLIC_AMAZON_AFFILIATE_TAG}`,
    pros: ["Large capacity", "Powerful motor", "Multiple blades", "Easy to clean"],
    cons: ["Large footprint", "Expensive"],
  },
  {
    name: "KitchenAid 13-Cup Food Processor",
    price: "$169.99",
    rating: 4.6,
    link: `https://www.amazon.com/s?k=KitchenAid+KFP1311+13+Cup+Food+Processor&tag=${process.env.NEXT_PUBLIC_AMAZON_AFFILIATE_TAG}`,
    pros: ["Exact Slice system", "Sleek design", "Multiple speeds", "Work bowl stabilizer"],
    cons: ["Heavy", "Complex assembly"],
  },
  {
    name: "Hamilton Beach 10-Cup Food Processor",
    price: "$49.99",
    rating: 4.5,
    link: `https://www.amazon.com/s?k=Hamilton+Beach+70725+10+Cup+Processor&tag=${process.env.NEXT_PUBLIC_AMAZON_AFFILIATE_TAG}`,
    pros: ["Affordable", "Compact", "Easy to use", "Dishwasher safe parts"],
    cons: ["Smaller capacity", "Less powerful"],
  },
  {
    name: "Breville Sous Chef 16-Cup",
    price: "$299.95",
    rating: 4.8,
    link: `https://www.amazon.com/s?k=Breville+BFP800XL+Sous+Chef+Processor&tag=${process.env.NEXT_PUBLIC_AMAZON_AFFILIATE_TAG}`,
    pros: ["Commercial-grade", "Multiple feed tubes", "Quiet motor", "Premium construction"],
    cons: ["Very expensive", "Large"],
  },
];

export default function BestFoodProcessor() {
  return (
    <div className="min-h-screen bg-stone-50">
      <div className="max-w-3xl mx-auto px-4 py-12">
        <Link href="/reviews" className="inline-flex items-center text-emerald-600 hover:text-emerald-700 mb-8 font-lato font-medium transition">← Back to Reviews</Link>
        
        <article>
          <div className="flex items-center gap-4 mb-4">
            <span className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-sm font-lato font-medium">Appliances</span>
            <span className="font-lato text-emerald-500 font-bold">★ 4.7</span>
            <span className="font-lato text-stone-400 text-sm">Feb 17, 2026</span>
          </div>
          
          <div className="rounded-2xl overflow-hidden mb-8 shadow-lg">
            <img 
              src="/images/reviews/food-processor.jpg" 
              alt="Best Food Processors" 
              className="w-full h-64 object-cover"
            />
          </div>
          
          <h1 className="font-playfair text-4xl text-stone-800 mb-6">Best Food Processors: Complete Guide</h1>
          <p className="font-lato text-xl text-stone-600 mb-8">From compact to commercial-grade, we tested food processors to find the best for every kitchen and budget.</p>
          
          <div className="prose prose-stone max-w-none font-lato">
            <h2 className="font-playfair text-2xl text-stone-800 mt-8 mb-4">What to Look For</h2>
            <p className="mb-4 text-stone-600">Consider motor power (600W minimum), bowl capacity (7-14 cups), and included attachments. Look for processors with multiple blade options and easy assembly.</p>
            
            <h2 className="font-playfair text-2xl text-stone-800 mt-8 mb-4">Our Top Pick: Cuisinart 14-Cup</h2>
            <p className="mb-4 text-stone-600">The Cuisinart offers the perfect balance of power, capacity, and price. It's versatile enough for most household tasks.</p>
          </div>
        </article>

        <section className="space-y-8 mt-12">
          {products.map((product, index) => (
            <div key={index} className="bg-white rounded-2xl p-6 shadow-sm border border-stone-200 hover:shadow-md transition">
              <div className="flex flex-col md:flex-row gap-6">
                <div>
                  <div className="flex justify-between items-start mb-2">
                    <h2 className="text-xl font-bold text-stone-900">{product.name}</h2>
                    <span className="text-2xl font-bold text-emerald-600">{product.price}</span>
                  </div>
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-amber-500">
                      {"★".repeat(Math.floor(product.rating))}
                      {"☆".repeat(5 - Math.floor(product.rating))}
                    </span>
                    <span className="text-stone-600">{product.rating}/5</span>
                  </div>
                  <ul className="mb-4 space-y-1">
                    {product.pros.map((pro, i) => (
                      <li key={i} className="text-stone-600 text-sm font-lato">✓ {pro}</li>
                    ))}
                  </ul>
                  <a 
                    href={product.link} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="inline-block bg-emerald-600 text-white px-6 py-2 rounded-lg font-lato font-medium hover:bg-emerald-700 transition"
                  >
                    Check Price on Amazon
                  </a>
                </div>
              </div>
            </div>
          ))}
        </section>
      </div>
    </div>
  );
}
